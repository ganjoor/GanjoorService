            ...
            _backgroundTaskQueue.QueueBackgroundWorkItem
              ...
              async token =>
              {
                  using (RMuseumDbContext context = new RMuseumDbContext(new DbContextOptions<RMuseumDbContext>())) //this is long running job, so _context might be already been freed/collected by GC
                  {
                      ...
                      var job = (await jobProgressServiceEF.NewJob($"OpenAIStartCreatingImagesForPoems - start: {startFrom} - count: {count} - poet: {poetId}", "Open AI initialization")).Result;
                      try
                      {
                          ...
                          var poems =
                            poetId != 0 ?
                            await context.GanjoorPoems.Include(p => p.Cat).AsNoTracking().Where(p => p.Cat.PoetId == poetId).ToListAsync() :
                            count == 0 ?
                            startFrom == 0 ?
                          ...
                          await jobProgressServiceEF.UpdateJob(job.Id, 0, $"Starting, {poems.Count}");
