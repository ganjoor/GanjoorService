...
namespace GanjooRazor.Pages
{
    ...
    public class SimiModel : LoginPartialEnabledPageModel
    {
        ...
        /// constructor
        /// </summary>
        public SimiModel(HttpClient httpClient, IConfiguration configuration, PoetCacheService poetCache) : base(httpClient, configuration)
        {
        ...
        public async Task<IActionResult> OnGetAsync()
        {
            ...
            if (CatId != 0)
            {
                ...
                {
                    LastError = await ReadErrorMessageAsync(catResponse);
                    return Page();
                }
                else
                {
            ...
            if (PaginationMetadata.totalPages > 1)
            {
                ...
                for (int i = PaginationMetadata.currentPage - 2; i <= (PaginationMetadata.currentPage + 2); i++)
                {
                    if (i >= 1 && i <= PaginationMetadata.totalPages)
                    {
                        ...
                        {
                            htmlText += $"<a href=\"/simi/?v={Uri.EscapeDataString(Metre)}&amp;g={Uri.EscapeDataString(Rhyme)}&amp;page={i}{authorParam}&amp;l={Language}&amp;f={(int)Format}&amp;c1={CoupletCountsFrom}&amp;c2={CoupletCountsTo}{exceptUrl}\"><div class=\"circled-number\">{i.ToPersianNumbers()}</div></a>{Environment.NewLine}";
                        }
                    }
                }
                if (PaginationMetadata.totalPages > (PaginationMetadata.currentPage + 2))
                {
                    htmlText += $"… <a href=\"/simi/?v={Uri.EscapeDataString(Metre)}&amp;g={Uri.EscapeDataString(Rhyme)}&amp;page={PaginationMetadata.totalPages}{authorParam}&amp;l={Language}&amp;f={(int)Format}&amp;c1={CoupletCountsFrom}&amp;c2={CoupletCountsTo}{exceptUrl}\"><div class=\"circled-number\">{PaginationMetadata.totalPages.ToPersianNumbers()}</div></a>{Environment.NewLine}";
                }
                htmlText += $"</div>{Environment.NewLine}";
            }
        ...
        public Task<IActionResult> OnPostSendSectionMetreSuggestionAsync(int poemId, int sectionIndex, string rhythm)
        {
            return WithSecureClientAsync(async secureClient =>
            {
                ...
                {
                    return new BadRequestObjectResult(await ReadErrorMessageAsync(sectionResponse));
                }
                ...
                {
                    return new BadRequestObjectResult(await ReadErrorMessageAsync(correctionResponse));
                }
